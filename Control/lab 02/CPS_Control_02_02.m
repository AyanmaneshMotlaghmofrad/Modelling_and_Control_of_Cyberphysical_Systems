...
noise = -5 + 10 * rand(1, 50); % gaussian noise assumption
...
for c = 1:N-2
    ...
    %initialization
    supp_eq = zeros(8, size_theta+ N);

    %theta rows
    supp_eq(2:5, 1:size_theta) = eye(size_theta); 
    supp_eq(7:end, 1:2) = eye(2);
    
    % eta rows
    supp_eq(6:end, size_theta+c:size_theta+c+2) = flip(eye(3));
...
[PUI_lbd' [theta_real(2:3)' theta_real(5:6)']' PUI_ubd']
...
% for c = N-1
    ...
    supp_eq = zeros(5,size_theta + N);
    supp_eq(2:end, 1:size_theta) = eye(4);

    coef_eq = [-100 -100 -100 1 1]'; 
    ineqPolySys{c+1}.noTerms = 5;
...
[PUI_lbd' PUI_lbd_dcgain' [theta_real(2:3)' theta_real(5:6)']'  theta PUI_ubd_dcgain' PUI_ubd']

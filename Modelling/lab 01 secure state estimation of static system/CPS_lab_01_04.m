...
for i = 1:run  
    ...
    % delta = 1e-4; %we don't need a very exact convergence
    lambda = 0.1;
    nu = 0.7;
    ...
    for k = 1:max_iteration
        ...
        if(norm(x_hat_my(:,k+1)-x_hat_my(:,k))^2<delta)  % condition for maximum iteration number
            break
        end
    ...
    %ISTA SUPPORT ERROR
    if(length(attack_hat_ISTA_idx)==0)
...
mean_last_col_IJAM = mean(last_col_IJAM)
mean_last_col_ISTA = mean(last_col_ISTA)
mean_last_col_my = mean(last_col_my)

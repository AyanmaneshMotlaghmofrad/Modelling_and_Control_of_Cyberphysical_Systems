...
state_estimation_error_my_IJAM = zeros(1,run); 
state_estimation_error_my_ISTA = zeros(1,run); 

attack_support_error_IJAM = zeros(1,run);
attack_support_error_ISTA = zeros(1,run);
attack_support_error_my_IJAM = zeros(1,run);
attack_support_error_my_ISTA = zeros(1,run);
...
for i = 1:run  
    ...
    last_col_my_IJAM(i) = find(any(x_hat_my_IJAM, 1), 1, 'last');
    ...
    subplot(2,2,3);
    for j = 1:length(x_tilde)
        plot(x_hat_my_IJAM(j,1:last_col_my_IJAM(i)))
        hold on,grid on
        plot(x_tilde(j)*ones(1,last_col_my_IJAM(i)),'--k')
        title('x hat my_IJAM')
    end
    ...
    a_hat_my_ISTA = zeros(q,max_iteration);lambda
    %try initializing x with pinv(G)*C
    ...
    % still one step needs to be done so these numbers are incremented by 1
    % at the end of the process.
    last_col_my_ISTA(i) = find(any(x_hat_my_ISTA, 1), 1, 'last');
    ...
    subplot(2,2,4);
    for j = 1:length(x_tilde)
        plot(x_hat_my_ISTA(j,1:last_col_my_ISTA(i)))
        hold on,grid on
        plot(x_tilde(j)*ones(1,last_col_my_ISTA(i)),'--k')
        title('x hat my_ISTA')
    end
    % states comparison (just for checking if it works)
    ...
    state_estimation_error_my_IJAM(i) = norm(x_hat_my_IJAM(:,last_col_my_IJAM(i))-x_tilde)/norm(x_tilde);
    state_estimation_error_my_ISTA(i) = norm(x_hat_my_ISTA(:,last_col_my_ISTA(i))-x_tilde)/norm(x_tilde);
    

    attack_idx = find(a_tilde)
    attack_hat_IJAM_idx = find((a_hat_IJAM(:,last_col_IJAM(i))))
    attack_hat_ISTA_idx = find((a_hat_ISTA(:,last_col_ISTA(i)))) 
    attack_hat_my_IJAM_idx = find(abs(a_hat_my_IJAM(:,last_col_my_IJAM(i))))
    attack_hat_my_ISTA_idx = find(abs(a_hat_my_ISTA(:,last_col_my_ISTA(i))))
    ...
    %my_IJAM SUPPORT ERROR
    for j = 1:h          
        if attack_idx(j) ~= attack_hat_my_IJAM_idx(j)
            attack_support_error_my_IJAM(i) =attack_support_error_my_IJAM(i) +1;  
        end                                      
    end

    %my_ISTA SUPPORT ERROR

    for j = 1:h          
        if attack_idx(j) ~= attack_hat_my_ISTA_idx(j)
            attack_support_error_my_ISTA(i) =attack_support_error_my_ISTA(i) +1;  
        end                                      
    end
...
mean_iteration_my_IJAM = mean(last_col_my_IJAM)
mean_iteration_my_ISTA = mean(last_col_my_ISTA)
...
mean_attack_support_error_my_IJAM = mean(attack_support_error_my_IJAM)
mean_attack_support_error_my_ISTA = mean(attack_support_error_my_ISTA)
...
mean_state_estimation_error_my_IJAM = mean(state_estimation_error_my_IJAM)
mean_state_estimation_error_my_ISTA = mean(state_estimation_error_my_ISTA)


my_IJAM_precision_vs_IJAM = (mean_state_estimation_error_IJAM-mean_state_estimation_error_my_IJAM)...
                        /mean_state_estimation_error_IJAM*100
my_ISTA_precision_vs_ISTA = (mean_state_estimation_error_ISTA-mean_state_estimation_error_my_ISTA)...
                        /mean_state_estimation_error_ISTA*100

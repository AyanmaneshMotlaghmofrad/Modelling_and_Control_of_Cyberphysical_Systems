...
h = 8; % two sensors are under attack.
...
for i = 1:run  
    ...
    S_est  = find(a_hat_IJAM(:, k) ~= 0);
    TP = length(intersect(S_true, S_est));
    support_recovery_rate_IJAM(i) = TP / length(S_true);
    ...
    S_est  = find(a_hat_ISTA(:, k) ~= 0);
    TP = length(intersect(S_true, S_est));
    support_recovery_rate_ISTA(i) = TP / length(S_true);
...
mean_state_estimation_error_my_IJAM(last_non_zero_my_IJAM)
mean_state_estimation_error_IJAM(last_non_zero_IJAM)
mean_state_estimation_error_my_ISTA(last_non_zero_my_ISTA)
mean_state_estimation_error_ISTA(last_non_zero_ISTA)

mean_support_recovery_rate_IJAM(counter) = mean(support_recovery_rate_IJAM)
mean_support_recovery_rate_ISTA(counter) = mean(support_recovery_rate_ISTA)
